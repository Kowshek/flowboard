...
function SettingsSection({ icon: Icon, title, description, children }: {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm dark:shadow-none dark:border dark:border-white/10 overflow-hidden transition-colors">
      <div className="px-6 py-5 border-b border-gray-100 dark:border-white/[0.06] flex items-center gap-3">
        <div className="h-8 w-8 rounded-lg bg-violet-50 dark:bg-violet-500/10 flex items-center justify-center flex-shrink-0">
...
function ToggleRow({ label, description, enabled }: { label: string; description: string; enabled?: boolean }) {
  return (
    <div className="flex items-center justify-between py-3 border-b border-gray-50 dark:border-white/[0.04] last:border-0">
      ...
      <div className={`relative w-9 h-5 rounded-full transition-colors ${enabled ? "bg-violet-600" : "bg-gray-200 dark:bg-gray-700"}`}>
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      <SettingsSection icon={Bell} title="Notifications" description="Choose what you want to be notified about">
        <ToggleRow label="Product updates" description="New features and improvements" enabled />
        <ToggleRow label="Weekly digest"   description="Summary of your key metrics every Monday" enabled />
        <ToggleRow label="Churn alerts"    description="Notify me when a customer churns" />
      </SettingsSection>
      ...
      <SettingsSection icon={CreditCard} title="Billing" description="Your subscription and usage">
